...
public class CraftersTriggerScript : MonoBehaviour
{
	...
	private void OnTriggerEnter(Collider other)
	{
		if (other.tag == "Player")
		{
			...
			if (GameControllerScript.current.mode == "story_double")
				FindObjectOfType<BaldiCharacterManager>().crafters.GiveLocation(this.goTarget.position, false);
		}
	}
	...
	private void OnTriggerExit(Collider other)
	{
		if (other.tag == "Player")
		{
			...
			if (GameControllerScript.current.mode == "story_double")
				FindObjectOfType<BaldiCharacterManager>().crafters.GiveLocation(this.fleeTarget.position, true);
		}
	}
